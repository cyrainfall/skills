...
description: 审计并优化现有 Qoder 技能，使其简洁、层次清晰、渐进式披露，且流程描述完备、语言规范、异常处理健全。当 SKILL.md 接近或超过 500 行、引用文件缺少明确指引、大型引用文件（>300行）需要目录、流程描述缺少五要素（触发条件/主流程/分支/异常/输出）、语言含模糊词、异常处理无错误码或降级路径，或用户要求重构、精简、重组或改进技能时使用。
---
...
```bash
python .qoder/skills/skill-optimizer/scripts/check_skill.py <技能目录路径>
```
